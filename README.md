...
- 🌅 **아침/저녁 맞춤형 GTD 브리핑 (Morning & Evening Briefing - ADR-024)**
  ...
  - **듀얼 엔진 & 원터치 칩**: LLM 지능형 합성과 0.01초 무지연 결정론적 룰 기반 포맷터 듀얼 엔진으로 무결점 응답을 보장하며, 웹 콘솔에 `[🌅 아침 브리핑]`, `[🌇 저녁 회고]` 원터치 칩 바 및 `GET /api/briefing?mode=morning|evening` REST API를 제공합니다.
