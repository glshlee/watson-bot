...
- 📱 **모바일 뷰포트 레이아웃 안정화 & 컴포넌트 충돌 방지 (Mobile Layout Stabilization - ADR-023)**
  ...
  - **하단 입력창 50% 슬림화**: 불필요한 이중 패딩/테두리를 제거한 슬림 가로 스와이프 필 툴바로 칩 바를 일체화하고, 입력창 높이를 압축하여 하단 고정 영역을 140px에서 75px로 절반 다이어트했습니다.
  ...
  - **포털 2x2 메트릭스 & 캐시 방지**: 에이전트 허브 포털의 메트릭 카드 4종을 2x2 컴팩트 모바일 그리드로 재배치하고, 정적 자산 캐시 방지 쿼리 `v=1.3.2`를 적용했습니다.
