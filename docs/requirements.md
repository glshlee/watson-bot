...
- **FR-22.3**: 하단 빠른 칩 바(`.watson-quick-bar`, `.dev-quick-bar`)를 불필요한 테두리/패딩 없는 슬림 가로 스와이프 필 바(`scrollbar-width: none`)로 개편하고, 카테고리 셀렉트 및 텍스트 입력창 높이를 축약하여 하단 고정 높이를 140px에서 75px 수준으로 50% 슬림화해야 한다.
...
- **FR-22.5**: 에이전트 허브 포털의 메트릭 카드를 모바일에서 2x2 반응형 그리드로 정돈하고, 정적 CSS 캐시 방지를 위해 `v=1.3.2` 쿼리 파라미터를 적용해야 한다.
