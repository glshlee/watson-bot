...
* **모바일 뷰포트 레이아웃 안정화 & 컴포넌트 충돌 방지 (ADR-023)**: 모바일(<=768px) 헤더 긴 부제목 숨김, 우측 배지 34x34px 아이콘화 및 GTD 배지 축약으로 상단 겹침을 방지하고, 하단 입력창 50% 슬림화(140px➔75px) 및 Flex Overflow(min-width:0) 차단으로 화면 흔들림과 컴포넌트 밀림을 원천 차단할 것.
